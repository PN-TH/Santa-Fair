...
export class PlacesRepository {
    ...
    private static instance: PlacesRepository;
    ...
    static getInstance() {
        if (!this.instance) {
            this.instance = new PlacesRepository();
        }
    ...
    findAll(): Promise<Place[]> {
        return this.connection.query(`SELECT * from ${this.table}`)
          .then((results: any) => {
            return results.map((post: any) => new Place(post));
          });
    }
    ...
    findById(id: number): Promise<Place> {
        return this.connection.query(`SELECT * FROM ${this.table} WHERE id = ?`, [id])
          .then((results: any) => new Place(results[0]));
    }
